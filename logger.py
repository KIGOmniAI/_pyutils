...
class Logger():
  ...
  def __init__(self, lib_name = "LOGR", lib_ver = "",
    ...
    """
     ...
     "quick" methods:
       PrintPad(msg1, msg2, n=3) - prints msg1 in normal line and msg2 (multi-line) indented with n spaces
       ...
       CheckFolder(sub_folder) - creates sub_folder if does not exists in repository
       
       GetFileFromFolder(sub_folder, file)
       
       GetDataFile(file) - returns (if exists) full path to _data folder file
       GetOutputFile(file) - returns (if exists) full path to _output folder file
       GetGoogleDrive()
       
       CleanConvertModel() - SAVES KERAS MODEL CLEAN - WITHOUT learnin_phase 
       
       SaveModelAsGraph() - saves keras model as graph pb file
       ...
       GetStandardKerasCallbacks() - returns epoch and checkpoin callbacks
       
       GetKerasModelSummary(model, full_info = False)
  ...
  def SaveGraph(self, session, tensor_list, 
    ...
    tensor_list : list of ouput tensors
    pb_file     : .pb filename
